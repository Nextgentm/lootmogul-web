...
const FounderNfts = ({ data }) => {
    ...
    useEffect(() => {
        ...
        let sliderItem = featured.map((nftData) => {
            return  nftData.nft_kred && nftData?.nft_kred?.data && (<Box w={"100%"}>
                   <Box mt ="5%">
                    
                       <NftCard isMobileDevice = {isMobileDevice} nft={nftData?.nft_kred?.data} />
    ...
    return (
        <Box pos="relative" >
 <Box position="relative" align="center" w="100%" h={["460px","580px"]}>
             <Box width="100%" height="100%">  
             {data[0]?.banner.data &&  <Image
                    ...
                    src={
                        isMobileDevice
                            ...
                            ? getStrapiMedia(data[0]?.banner.data[0].url)
            ...
            <Flex dir="row" right={["10%","10%","10%","0%"]} top={["45%","45%","45%","10%"]}  pos="absolute" width="80%" bg="transparent" justifyContent={["center","center","center","flex-end"]}>
                   
                   {featuredNfts?.length > 0 && (
                      <Box   position="relative"  right={["0%","0%","0%","3%"]} width={["100%","100%","100%","50%"]}>
         ...
         {data && data.map((foundersnft,index)=>{
           return  <Box key={"founders"+index} mx={!isTabletOrDesktop ? 6 : 10} mt={8}>
                <Grid
                ...
                <Wrap m="auto !important">
                    {foundersnft.nftSet
                        ...
                        .map((item,index) => (
                            item.nft_kred && item.nft_kred.data &&  <WrapItem mb="3%!important" mt="auto" mx="auto!important"  >
                                <NftCard isMobileDevice = {isMobileDevice} nft={item?.nft_kred?.data} showInfo={true} />
                            </WrapItem>
                        ))}

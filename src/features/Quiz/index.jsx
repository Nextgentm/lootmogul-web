...
import {getVoiceOverStatus, setVoiceOverStatus} from "../../services/audioService";
const Quiz= (props) => {
  ...
  const [audio, setAudio] = useState(typeof Audio !== "undefined" && new Audio("/assets/audio/home/home_sound.mp3"));
  
  const { getSocket, setGameInProgress,setIsHideHeader,setIsHideFooter,updateUser} = useContext(AppContext);
    ...
    socket.on("tick", data => {
      ...
      socket.on("player_answered", data => {
        ...
        // setUsers(data.users.sort((a, b) => (a.rank > b.rank ? 1 : -1)));
          ...
          let alloptions = [[],[],[],[]];
          data.users.map(user=>{
            if(user.answers.length>lastQuestionNo && user.answers[lastQuestionNo].option){
             let optionNo = user.answers[lastQuestionNo].option - 1;  // because answers are 1-4 but indexes are 0 to 3
              if(optionNo >= 0){
...
const handleOptionClicked = (index, option) => {
  ...
  if(selOption>=0) return; // cannot select twice
  if(correctAns>=0) return; // error cannot select now as answer is out
  if(socket){
    socket.emit("submit_answer", {"currentQuestionNo":currentQuestionNo, "answer":option}); 
  }
  setSelOption(index); 
    ...
    <>
      ...
      >
        ...
        <Box
          ...
          <Box >
          {users?.length && ( <Players users={users} showAnswer={correctAns>=0} currentQuestionNo={currentQuestionNo}/>) }
          </Box>
        </Box>
        <Box
